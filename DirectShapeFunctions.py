...
from Autodesk.Revit.DB import Curve, CurveLoop, GeometryCreationUtilities, Line, SolidOptions, SolidUtils, SubfaceType, Transform, XYZ
            ...
            if not inserts_id_list:
                ...
                for insert_id in inserts_id_list:
                    ...
                    if item.GetType().Name == "FamilyInstance":
                        solid = get_wall_cut(
                            current_doc, item, b_element, face)
                    elif item.GetType().Name == "Wall":
                        solid = get_wall_profil(item, b_element, face)
                    else:
                        solid = bbox_to_solid(
                            item.get_BoundingBox(current_doc.ActiveView))
...
def get_wall_cut(current_doc, item, wall, _face):
    ...
    try:
        ...
        geom = GeometryCreationUtilities.CreateExtrusionGeometry(
