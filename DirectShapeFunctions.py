...
from Autodesk.Revit.DB import BooleanOperationsUtils, BooleanOperationsType, Curve, CurveLoop, GeometryCreationUtilities, Line, SolidOptions, SolidUtils, SubfaceType, Transform, XYZ
...
def main_face_filter(room, host_id, link_id, current_doc, link_doc, face, wall_type_names_to_exclude, transformer):
	...
	if host_id != id_minus_one:
		if is_not_curtain_modelline(host_id, current_doc):
			...
			ds_type_material = get_wall_ds_type_material(current_doc, b_element, room)
			inserts_solid_by_wall = get_inserts_solid_cuboid_from_wall(
				b_element, current_doc, current_doc, face, wall_type_names_to_exclude)
	elif link_id != id_minus_one:
		if is_not_curtain_modelline(link_id, link_doc):
			b_element = link_doc.GetElement(link_id)
			ds_type_material = get_wall_ds_type_material(link_doc, b_element, room)
			inserts_solid_by_wall = get_inserts_solid_cuboid_from_wall(
				b_element, current_doc, link_doc, face, wall_type_names_to_exclude)
			inserts_solid_by_wall = transformer.transform_to_current_doc(
				inserts_solid_by_wall)
...
def get_inserts_solid_cuboid_from_wall(b_element, current_doc, _doc, face, wall_type_names_to_exclude):
	...
	if b_element.GetType().Name == "Wall":
		...
		if type_name in wall_type_names_to_exclude:
			...
			inserts_id_list = b_element.FindInserts(
				incopenings, incshadows, incwalls, incshared)
...
#             pass
#         else:
#             inserts_list = b_element.FindInserts(
#                 incopenings, incshadows, incwalls, incshared)
#             if not inserts_list:
#                 pass
#             else:
...
#                         else:
#                             x = BoundingBox.ToCuboid(item.get_BoundingBox(
#                                 current_doc.ActiveView).ToProtoType())
...
def get_wall_ds_type_material(current_doc, b_element, room, concrete_mat_prfx="бетон"):
	...
	if current_doc.GetElement(b_element.GetTypeId()) and current_doc.GetElement(b_element.GetTypeId()).get_Parameter(BuiltInParameter.STRUCTURAL_MATERIAL_PARAM):
		mat_name = current_doc.GetElement(b_element.GetTypeId()).get_Parameter(
			BuiltInParameter.STRUCTURAL_MATERIAL_PARAM).AsValueString()
		if concrete_mat_prfx in str(mat_name).lower():
...
#         try:
#             ds_type = get_wall_type_material(current_doc, current_doc.GetElement(
#                 boundarylist[index + 1].ElementId), room)
#         except:
#             pass
#     elif index - 1 >= 0:
#         try:
